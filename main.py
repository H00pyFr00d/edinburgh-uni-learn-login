...
try:
    ...
    with open(os.getcwd() + "\\userinfo.txt", "w") as f:
        while True:
            ...
            if (input("Is " + username + " correct? (y/n): ").lower().strip() == "y") & (len(username) == 8) & (username[0] == "s"):
                f.write(username)
                break
        f.close()
...
WebDriverWait(driver, 3).until(EC.presence_of_element_located((By.XPATH, "//*[(@id = \"password\")]"))).send_keys(
    keyring.get_password("MyEd", username))
WebDriverWait(driver, 3).until(EC.presence_of_element_located((By.XPATH, "//*[(@id = \"submit\")]"))).click()

# if we ended up at this page again we've got a wrong user or password
if driver.current_url.find("https://www.ease.ed.ac.uk/cosign.cgi"):
